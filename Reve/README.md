...
### ✅ 쿠폰 적용 UX 리뉴얼 (라디오 + 모달 확정)

- 기존 "적용/해제" 버튼 제거
- 보유 쿠폰 목록은 라디오로 표시(초기 상태: 미적용)
- 쿠폰 선택 시 즉시 적용하지 않고 ConfirmModal로 최종 확정
   - 확인: couponStore.apply(code) 적용
   - 취소: store 상태 기준으로 paint() 재렌더하여 선택 롤백
- 이미 적용된 쿠폰을 다시 선택하면 "해제할까요?" 모달
   - 확인: couponStore.clearApplied()
   - 취소: 적용 유지 + paint()로 UI 복구

> 설계 의도: 결제 직전 실수(잘못된 쿠폰 적용/해제)를 방지

### ✅ Cart 사이즈 변경 UX (모달 + 디테일 토스트)

- Cart에서도 ProductCard와 동일한 사이즈 pill UI 제공
- pill 클릭 시 ConfirmModal로 변경 여부 확인(실수 방지)
   - 확인: cartStore.updateOptions(key, { size })로 변경
   - 동일 옵션 라인이 이미 있으면 qty 병합(중복 라인 제거)
- 변경 결과 토스트를 디테일하게 표시
   - A → B 변경 / 병합 발생 여부 안내
- 취소 시 UI는 paint()로 즉시 원상복구

### ✅ 구매하기(Checkout) UX 강화

- 구매하기 클릭 시 결제 확인 모달 표시
   - 확인: Mock 결제(handleCheckout) 진행
   - 취소: 결제 취소(상태 유지)
- 결제 성공 시 “결제 완료” 모달로 요약 제공
   - 사용 쿠폰/배송비/최종 결제금액 표시
- 결제 실패 시 토스트 안내
